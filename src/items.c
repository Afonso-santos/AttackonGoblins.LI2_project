...
int whats_around(Position pos, char **map){    
    ...
    for (int i = -1; i <2 ; i++){
        for (int j = -1; j <2 ; j++){
            if(map[pos.x][pos.y]==enemy_char){
                return 1;

            }else if(map[pos.x][pos.y]==player_char){
                return 2;

            }else if(map[pos.x][pos.y]==axe_char || map[pos.x][pos.y]==spear_char || map[pos.x][pos.y]==Flashlight){
                return 3;          
            }
        }
    }
