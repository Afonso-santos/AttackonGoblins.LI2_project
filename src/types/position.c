...
Position *position_get_up(Position *position) {
    Position *up = create_position(position->x, position->y + 1);
    return up;
}

Position *position_get_down(Position *position) {
    Position *down = create_position(position->x, position->y - 1);
    return down;
}

Position *position_get_left(Position *position) {
    Position *left = create_position(position->x - 1, position->y);
    return left;
}

Position *position_get_right(Position *position) {
    Position *right = create_position(position->x + 1, position->y);
    return right;
}
